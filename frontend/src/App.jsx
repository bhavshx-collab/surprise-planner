...
export default function App() {
  ...
  return (
    <div className="app">
      ...
      {/* Landing page — NO navbar shown here, it has its own */}
      {view === "landing" && (
        <LandingPage
          onPlan={() => setView("app")}
      ...
      {/* Everything else — show the navbar */}
      {view !== "landing" && (
        <>
          <nav className="navbar">
            ...
            <div className="navbar-right">
              ...
              <button className="nav-btn" onClick={() => setView("vendors")}>Find vendors</button>
              {user ? (
                <div style={{ position: "relative" }}>
                  ...
                  {showMenu && (
                    <div style={{
                      ...
                      <button onClick={() => { setView("myplans"); setShowMenu(false); }} style={menuItemStyle}>My plans</button>
                      <button onClick={() => { setView("vendordashboard"); setShowMenu(false); }} style={menuItemStyle}>My business</button>
                      {user.email === "bhaveshkumawat330@gmail.com" && (
                        <button onClick={() => { setView("admin"); setShowMenu(false); }} style={menuItemStyle}>Admin panel</button>
                      )}
                      <button onClick={() => { setView("vendorsignup"); setShowMenu(false); }} style={menuItemStyle}>List my business</button>
                      <div style={{ borderTop: "0.5px solid #eee", marginTop: "4px", paddingTop: "4px" }}>
                        <button onClick={() => { supabase.auth.signOut(); setShowMenu(false); }} style={{ ...menuItemStyle, color: "#E24B4A" }}>Sign out</button>
                      </div>
                    </div>
                  )}
                </div>
          ...
          {view === "myplans" && user && <MyPlans user={user} onSelectPlan={(p) => { setResult(p.plan_data); setStep(4); setView("app"); }} onNewPlan={() => { handleReset(); setView("app"); }} />}
          {view === "vendorsignup" && <VendorSignup onBack={() => setView("app")} />}
          {view === "vendordashboard" && user && <VendorDashboard user={user} onBack={() => setView("app")} />}
          {view === "admin" && <AdminDashboard user={user} onBack={() => setView("app")} />}
          {view === "vendors" && <VendorDirectory city={form.city} onBack={() => setView("app")} onVendorSignup={() => setView("vendorsignup")} />}
          {view === "gallery" && <InspirationGallery onBack={() => setView("app")} onUsePlan={(p) => { setResult(p.plan_data); setStep(4); setView("app"); }} />}

          {/* Main app */}
          {view === "app" && (
            <>
              {/* ... all your existing hero, steps, result code ... */}
            </>
          )}
        </>
      )}
